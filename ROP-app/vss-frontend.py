...
def main():
    ...
    if uploaded_files:
        ...
        if st.session_state.show_gradcam and st.session_state.backend_url:
            ...
            try:
                response = requests.post(st.session_state.backend_url, files=files, data=data)
        ...
        # Display probabilities and diagnosis
        if probabilities and diagnosis:
            ...
            with metric_col1:
                st.metric(label="Predicted VSS", value=np.argmax(probabilities) + 1)
            with metric_col2:
