...
def generate_heatmap(probabilities):
    ...
    sns.heatmap([probabilities], cmap="YlOrRd", annot=True, fmt=".2f", cbar=False, xticklabels=range(1, 10), yticklabels=["VSS"])
    
    st.markdown("<div class='full-width-container'><div class='heatmap-container'>", unsafe_allow_html=True)
    st.pyplot(fig)
...
def main():
    ...
    # File uploader
    uploaded_files = st.file_uploader("Upload Fundus Images", type=["jpg", "png", "jpeg"], accept_multiple_files=True)
    ...
    if uploaded_files:
        ...
        st.image(current_image, caption=f'{image_name} (Image {st.session_state.image_index + 1} of {num_images})', use_container_width=True)
