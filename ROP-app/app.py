...
def draw_bounding_boxes(image, bounding_boxes):
    ...
    for box in bounding_boxes:
        ...
        cv2.rectangle(
            image, (x1, y1 - text_height - 4), (x1 + text_width, y1), color, -1
        )  # Text background
        cv2.putText(
            image,
            text,
...
def vote():
    ...
    if st.button("Save"):
        st.session_state.img_flask_api_url = "{}/OpticDisc-segmentation".format(
            link
        )  # Update ngrok URL
...
def main():
    ...
    if uploaded_img and example_button:
        ...
        if response.status_code == 200:
            ...
            base64_mask_string = (
                api_response["segmentation_masks"][0]
                if isinstance(api_response["segmentation_masks"], list)
                else api_response["segmentation_masks"]
            )
